...
def work_loop(interval=1):
    ...
    while True:
        if time.time() > deadline:
            log.info("{} units of work done, updating hash counter" #pylint: disable=logging-format-interpolation
                     .format(loops_done))
...
def work_once():
    ...
    if not hex_hash.startswith('0'):
        ...
        return
    log.info("Coin found: {}...".format(hex_hash[:8])) #pylint: disable=logging-format-interpolation
